...
class TileLocation : IdentityMapper.Class() {
    ...
    @DependsOn(TileLocation.set::class)
    class plane : OrderMapper.InMethod.Field(TileLocation.set::class, 0, 3) {
        override val predicate = predicateOf<Instruction2> { it.opcode == Opcodes.PUTFIELD && it.fieldType == INT_TYPE }
    }

    @DependsOn(TileLocation.set::class)
    class x : OrderMapper.InMethod.Field(TileLocation.set::class, 1, 3) {
        override val predicate = predicateOf<Instruction2> { it.opcode == Opcodes.PUTFIELD && it.fieldType == INT_TYPE }
    }

    @DependsOn(TileLocation.set::class)
    class y : OrderMapper.InMethod.Field(TileLocation.set::class, 2, 3) {
        override val predicate = predicateOf<Instruction2> { it.opcode == Opcodes.PUTFIELD && it.fieldType == INT_TYPE }
    }
}